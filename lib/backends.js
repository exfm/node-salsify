...
function SQS(){
    ...
    this.log = createLog('salsify.backend.sqs');
}
...
SQS.prototype.configure = function(parent, cb){
    ...
    aws.onConnected(function(){
        this.log.silly('Connected to AWS');
        cb();
    });
};
...
SQS.prototype.listen = function(q, cb){
    ...
    queue.on('message', function(message){
        self.log.silly('Got message: ' + util.inspect(message.body, true, 5, false));
...
function Memory(){
    this.queues = {};
    this.log = createLog('salsify.backend.memory');
}
